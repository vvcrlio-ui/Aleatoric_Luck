...
if [ "${ALLOW_PRE_TABLE_SUBMISSION:-0}" != "1" ]; then
  echo "Refusing the pre-table Slurm submission path: it bypasses the current dynamic task plan and execution protocol." >&2
  ...
  echo "Build a dynamic flat-task plan, then use slurm/submit_flat_task_table.sh [--submit] PLAN.json." >&2
