...
def test_b1_b3_old_cv_oracle(library, kind):
    ...
    if library == 'xgboost':
        ...
        old = lib.cv(candidate.params_, data, num_boost_round=rounds, folds=folds, seed=seed,
                     verbose_eval=False)['test-rmse-mean'].to_numpy()
        selected = int(np.argmin(old)) + 1
        oracle = lib.train(candidate.params_, data, num_boost_round=selected).predict(data)
    else:
        ...
        old = lib.cv(candidate.params_, lib.Dataset(X, label=y), num_boost_round=rounds,
            nfold=5, stratified=False, seed=seed,
            callbacks=[lib.early_stopping(3, verbose=False)])['valid rmse-mean']
        selected = int(np.argmin(old)) + 1
        oracle = lib.train(candidate.params_, lib.Dataset(X, label=y), num_boost_round=selected).predict(X)
