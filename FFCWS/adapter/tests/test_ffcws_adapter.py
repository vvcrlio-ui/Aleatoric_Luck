...
class PipelineTest(unittest.TestCase):
    def test_end_to_end_outputs_and_content_identity_are_deterministic(self):
        ...
        for offset, outcome in enumerate(DEFAULT_OUTCOMES):
            ...
            train.loc[0, outcome] = np.nan
            test.loc[0, outcome] = np.nan
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            config = {
                ...
                "exchangeability_justification": (
                    "Synthetic training-pool sources are exchangeable test units."
                ),
                "schema": {
                    "min_valid_rate": 0.5,
            ...
            self.assertEqual(first["eligible_raw_source_count"], 2)
            for strategy in config["strategies"]:
                ...
                self.assertEqual(int(loaded.train["gpa"].isna().sum()), 1)
                self.assertEqual(int(loaded.test["gpa"].isna().sum()), 1)
