...
def ensure_unique_ids(frame: pd.DataFrame, id_column: str, label: str) -> None:
    ...
    missing = frame[id_column].isna()
    if bool(missing.any()):
        raise ValueError(f"{label} has {int(missing.sum())} missing {id_column} values")
...
def ensure_disjoint_ids(
    ...
    *,
    ...
    if overlap:
        ...
        raise ValueError(
            f"train and test IDs overlap in {id_column}; example: {example}"
        )
