...
# Classification—command template; first provide a real binary 0/1 outcome column
...
Use `--n-jobs` to compute multiple `(model, seed, draw, N, K)` combinations at
the same time. The default is 4 parallel workers:

```bash
python src/nk_grid.py --task regression --outcome Cm_lhourlywage \
  --models xgboost ridge lasso --n-jobs 4
```

Run `python src/nk_grid.py --help` to view all arguments, or see “Notes” for the
...
```bash
python src/run_panels.py --dry-run          # Preview configurations and estimated run sizes
python src/run_panels.py                    # Run all panels in panels.yaml
python src/run_panels.py --only smr_hourlywage  # Run one named panel only
```
