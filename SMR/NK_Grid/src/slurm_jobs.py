...
try:
    from .experiment import file_sha256, utc_now, write_json_atomic
...
def write_submission_receipt(
    ...
    *,
    ...
    project_dir = Path(
        os.environ.get("PROJECT_DIR", worker_script.parent.parent)
    ).resolve()
    venv = Path(os.environ.get("VENV", project_dir / ".venv")).resolve()
    ...
    rerun_command = shlex.join(
        [
            ...
            (
                "--export=ALL,"
                f"PROJECT_DIR={project_dir},VENV={venv},PYTHON={python}"
            ),
            "--array=<task-index>",
            str(worker_script),
            str(snapshot_path),
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument(
        "command", choices=("count", "list", "snapshot", "receipt", "run")
    )
    ...
    log_progress(
        ...
        f"index={args.index} panel={job.panel} model={job.model} out={job.config.out}"
    )
