...
if [[ ! "$JOB_ID" =~ ^[0-9]+$ ]]; then
  echo "Could not parse Slurm job ID from sbatch output: $SBATCH_OUTPUT" >&2
  exit 1
fi
echo "Submitted Slurm array job $JOB_ID (snapshot=$SNAPSHOT)"

RECEIPT_ARGS=(
  receipt
  --snapshot "$SNAPSHOT"
  --job-id "$JOB_ID"
  --array-spec "0-$ARRAY_MAX"
  --worker-script "$PROJECT_DIR/slurm/run_nk_grid.sbatch"
)
if [ "$ALLOW_LARGE_RUN" = "1" ]; then
  RECEIPT_ARGS+=(--allow-large-run)
fi
if RECEIPT=$("$PYTHON" "$PROJECT_DIR/src/slurm_jobs.py" "${RECEIPT_ARGS[@]}"); then
  echo "Saved Slurm submission receipt: $RECEIPT"
else
  RECEIPT_STATUS=$?
  echo \
    "WARNING: job $JOB_ID was submitted, but its receipt could not be written (exit=$RECEIPT_STATUS)." \
    >&2
  echo "Recreate the receipt with:" >&2
  printf '  %q %q' "$PYTHON" "$PROJECT_DIR/src/slurm_jobs.py" >&2
  printf ' %q' "${RECEIPT_ARGS[@]}" >&2
  printf '\n' >&2
fi
