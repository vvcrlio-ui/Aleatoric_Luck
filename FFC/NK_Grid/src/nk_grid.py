...
def external_test_split(
    ...
    """Construct one fixed train/test split from independent input frames."""

    if outcome not in train_frame:
        raise KeyError(f"Outcome not found in training data: {outcome}")
    if outcome not in test_frame:
        raise KeyError(f"Outcome not found in test data: {outcome}")

    missing_predictors = [column for column in predictors if column not in test_frame]
    if missing_predictors:
        raise ValueError(
            "External test data is missing predictor columns selected from "
            f"training data: {', '.join(missing_predictors)}"
        )

    train_observed = train_frame.dropna(subset=[outcome])
    test_observed = test_frame.dropna(subset=[outcome])
    predictor_list = list(predictors)
    return SplitData(
        X_train=train_observed.loc[:, predictor_list],
        X_test=test_observed.loc[:, predictor_list],
        y_train=train_observed[outcome],
        y_test=test_observed[outcome],
    )
...
def read_table(path: Path) -> pd.DataFrame:
    """Read a supported NK input table based on its file suffix."""
    ...
    raise ValueError(
        f"Unsupported NK input format '{suffix}' for {path}; "
        "supported suffixes are .csv, .parquet, and .pq"
    )
...
def _validate_classification_outcome(values: pd.Series, *, context: str) -> None:
    ...
    if not classes.issubset({0, 1, False, True}) or len(classes) > 2:
        raise ValueError(
            ...
            f"Invalid classes found in {context}; pass the confirmed column "
            "via --outcome."
        )
...
def run_nk_grid(config: NKGridConfig, *, max_jobs: int | None = None) -> None:
    ...
    external_mode = config.test_data is not None
    test_path = Path(config.test_data) if config.test_data is not None else None
    test_frame = None
    if external_mode:
        if not test_path.exists():
            raise FileNotFoundError(f"External test data not found: {test_path}")
        test_frame = read_table(test_path)
        if config.outcome not in frame:
            raise KeyError(f"Outcome not found in training data: {config.outcome}")
        if config.outcome not in test_frame:
            raise KeyError(f"Outcome not found in test data: {config.outcome}")
    elif config.outcome not in frame:
    ...
    if external_mode:
        ...
        if not np.isclose(config.test_size, 0.3):
            log_progress(
                "external test_data mode ignores "
                f"test_size={config.test_size}; test_size is ignored because "
                f"the fixed test set comes from {test_path}"
            )
    log_progress(
        "loaded data "
    ...
    if external_mode:
        ...
        log_progress(
            ...
            f"train_observed={len(fixed_split.X_train)} "
            f"test_observed={len(fixed_split.X_test)}"
        )
    if config.task == "classification":
        _validate_classification_outcome(frame[config.outcome], context="training data")
        if external_mode:
            _validate_classification_outcome(
                test_frame[config.outcome], context="test data"
            )

    split_mode = "external_test" if external_mode else "internal_random"
    test_data_sha256 = file_sha256(test_path) if external_mode else None
    metadata_extra = {
    ...
    metadata = build_experiment_metadata(
        ...
        test_size=0.0 if external_mode else config.test_size,
